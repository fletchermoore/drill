# copy-paste of file from qt.aqt dyndeckconf
...
class DeckConf(QDialog):
    def __init__(self, mw, first=False, search="", deck=None):
        ...
        self.setWindowTitle(_("Drill Options for %s") % self.deck["name"])
        restoreGeom(self, "drilldeckconf")
        self.initialSetup()
        self.loadConf()
        if search:
            self.form.search.setText(search) # removed '+ " is:due"'
            self.form.search_2.setText(search + " is:new")
        self.form.search.selectAll()

        if self.mw.col.schedVer() == 1:
            self.form.secondFilter.setVisible(False)
        self.form.resched.setChecked(False) # don't reschedule drilling cards
        self.form.groupBox_2.setVisible(False) # don't show options
        self.show()
        self.exec_()
        saveGeom(self, "drilldeckconf")
